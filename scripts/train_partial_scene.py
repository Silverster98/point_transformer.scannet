...
from utils.dataset import ScannetDataset, ScannetDatasetWholeScene, collate_random, collate_wholescene
...
def get_dataloader(args, scene_list, phase):
    if args.use_wholescene:
        dataset = ScannetDatasetWholeScene(scene_list, npoints=args.npoints, is_weighting=not args.no_weighting, use_color=args.use_color, use_normal=args.use_normal, chunk_size=args.chunk_size)
        dataloader = DataLoader(dataset, batch_size=1, collate_fn=collate_wholescene, num_workers=args.num_workers, pin_memory=True)
    else:
        dataset = ScannetDataset(phase, scene_list, npoints=args.npoints, is_weighting=not args.no_weighting, use_color=args.use_color, use_normal=args.use_normal, chunk_size=args.chunk_size)
...
def train(args):
    ...
    solver, num_params = get_solver(args, dataset, dataloader, stamp, fea_dim, CONF.NUM_CLASSES)
