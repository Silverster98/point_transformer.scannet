...
class ScannetDataset():
    def __init__(self, phase, scene_list, num_classes=21, npoints=8192, is_weighting=True, use_color=False, use_normal=False, chunk_size=4):
        self.phase = phase
    ...
    def generate_chunks(self):
        ...
        for scene_id in tqdm(self.scene_list):
            ...
            for _ in range(5):
                ...
                curmin = curcenter-[self.chunk_size/2,self.chunk_size/2,1.5]
                curmax = curcenter+[self.chunk_size/2,self.chunk_size/2,1.5]
                curmin[2] = coordmin[2]
                curmax[2] = coordmax[2]
...
class ScannetDatasetWholeScene():
    def __init__(self, scene_list, npoints=8192, is_weighting=True, use_color=False, use_normal=False, chunk_size=4):
        self.scene_list = scene_list
        self.npoints = npoints
        self.is_weighting = is_weighting
        self.use_color = use_color
    ...
    def __getitem__(self, index):
        ...
        xlength = self.chunk_size
        ylength = self.chunk_size
...
class ScannetDatasetCompleteScene():
    def __init__(self, phase, scene_list, num_classes=21, npoints=8192, is_weighting=True, use_color=False, use_normal=False):
        self.phase = phase
        assert phase in ["train", "val", "test"]
        self.scene_list = scene_list
        self.num_classes = num_classes
...
def collate_random(data):
    ...
    semantic_seg = torch.LongTensor(np.array(semantic_seg))
    sample_weight = torch.FloatTensor(np.array(sample_weight))
...
def collate_wholescene(data):
    ...
    # convert to tensor
    point_sets = torch.FloatTensor(np.array(point_sets))
    semantic_segs = torch.LongTensor(np.array(semantic_segs))
    sample_weights = torch.FloatTensor(np.array(sample_weights))
